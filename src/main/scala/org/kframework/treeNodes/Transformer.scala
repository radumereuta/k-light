...
package org.kframework.treeNodes
...
abstract class ChildrenMapping[E, W] {
  ...
  def doTheMapping(t: HasChildren, f: (HasChildren, List[Term]) => Term): (Either[E, Term], W) = {
...
abstract class TransformerWithErrors[E] extends ChildrenMapping[E, Ignore] {
  ...
  override def mergeWarnings(a: Ignore, b: Ignore): Ignore.type = Ignore
  override val warningUnit: Ignore.type = Ignore
}
...
abstract class SafeTransformer extends ChildrenMapping[Ignore, Ignore] {
  ...
  def mergeWarnings(a: Ignore, b: Ignore): Ignore.type = Ignore
