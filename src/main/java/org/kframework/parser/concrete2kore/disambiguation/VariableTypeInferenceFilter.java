...
public class VariableTypeInferenceFilter extends SetsGeneralTransformer<ParseFailedException, ParseFailedException> {
    ...
    private Set<Sort> lowerBounds(Collection<Sort> bounds) {
        ...
        for (Sort sort : iterable(sortSet)) { // for every declared sort
            ...
            // as KList.
            if (subsorts.lessThanEq(sort, KORE.Sort("KBott")))
                continue;
            if (subsorts.greaterThan(sort, KORE.Sort("K")))
                continue;
    ...
    @Override
    public Tuple2<Either<Set<ParseFailedException>, Term>, Set<ParseFailedException>> apply(Term t) {
        Term loc = t;
    ...
    private class CollectVariables extends SetsGeneralTransformer<ParseFailedException, VarInfo> {
        public Tuple2<Either<Set<ParseFailedException>, Term>, Set<VarInfo>> apply(TermCons tc) {
            ...
            Tuple2<Either<Set<ParseFailedException>, Term>, Set<VarInfo>> rez = super.apply(tc);
            ...
            public Tuple2<Either<Set<ParseFailedException>, Term>, Set<VarInfo>> apply(TermCons tc) {
                if (tc.production().att().contains("bracket")
                        || (tc.production().klabel().isDefined()
                            && tc.production().klabel().get().equals(KORE.KLabel("#KRewrite")))) {
                   return super.apply(tc);
                }
                return simpleResult(tc);
            }

            public Tuple2<Either<Set<ParseFailedException>, Term>, Set<VarInfo>> apply(Constant c) {
    ...
    private class ApplyTypeCheck extends SetsTransformerWithErrors<ParseFailedException> {
        ...
        private class ApplyTypeCheck2 extends SetsTransformerWithErrors<ParseFailedException> {
            ...
            public Either<Set<ParseFailedException>, Term> apply(Constant c) {
                if (c.production().sort().equals(Sorts.KVariable())) {
            ...
            private Either<Set<ParseFailedException>, Term> wrapTermWithCast(Constant c, Sort declared) {
                Production cast;
                if (addCast) {
                    cast = productions.apply(KORE.KLabel("#SemanticCastTo" + declared.name())).head();
                } else if (!hasCastAlready) {
                    cast = stream(productions.apply(KORE.KLabel("#SyntacticCast"))).filter(p -> p.sort().equals(declared)).findAny().get();
                } else {
                    cast = null;
                }
                if (cast == null) {
