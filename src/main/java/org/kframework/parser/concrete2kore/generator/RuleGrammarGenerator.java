...
public class RuleGrammarGenerator {
    ...
    static {
        ...
        kSorts.add(KORE.Sort("RuleContent", ModuleName.apply("REQUIRES-ENSURES")));
    ...
    public static Definition autoGenerateBaseKCasts(Definition baseK) {
        return DefinitionTransformer.fromHybrid((Module m) -> {
            ...
            return Constructors.Module(m.name(),
                    m.imports(),
    ...
    public static Module getRuleGrammar(Module mod, Function<String, Module> getProcessedModule) {
        ...
        Module newM = new Module(mod.name() + "-" + RULE_PARSER, Collections.Set(mod, getProcessedModule.apply(K), getProcessedModule.apply(RULE_PARSER)), Collections.Set(), Att());
        return newM;
    }
    ...
    public static Module getConfigGrammar(Module mod, Function<String, Module> getProcessedModule) {
        ...
        Module newM = new Module(mod.name() + "-" + CONFIG_CELLS, Collections.Set(mod, getProcessedModule.apply(K), getProcessedModule.apply(CONFIG_CELLS)), Collections.Set(), Att());
        return newM;
    }
    ...
    public static Module getProgramsGrammar(Module mod, Definition baseK) {
        ...
        scala.collection.Set<Module> modules = Collections.Set(mod, baseK.getModule(PROGRAM_LISTS).get(), baseK.getModule(SORT_K).get());
        ...
        return Module.apply(mod.name() + POSTFIX, modules, Collections.Set(), Att());
    }
        ...
        return ModuleTransformer.fromHybrid((Module m) -> {
            ...
            if (seedMod.importedModules().exists(m1 -> m1.name().equals(K_TOP_SORT))) { // create the upper diamond
                for (Sort srt : iterable(m.localSorts())) {
                    if (!isParserSort(srt)) {
                        // K ::= Sort
                        newProds.add(Constructors.Production(Sorts.K(), Collections.Seq(NonTerminal(srt)), Att().add(Att.generatedByAutomaticSubsorting())));
                    }
                }
            }
            if (seedMod.importedModules().exists(m1 -> m1.name().equals(K_BOTTOM_SORT))) { // create the lower diamond
                for (Sort srt : iterable(m.localSorts())) {
                    if (!isParserSort(srt)) {
                        // Sort ::= KBott
                        newProds.add(Constructors.Production(srt, Collections.Seq(NonTerminal(Sorts.KBott())), Att().add(Att.generatedByAutomaticSubsorting())));
                    }
                }
            }
            if (newProds.isEmpty())
                ...
                if (disambM.importedModules().exists(m1 -> m1.name().equals(PROGRAM_LISTS)) && !m.name().equals(SORT_K)) {
                    ...
                    for (Sort srt : iterable(m.localSorts())) {
                        if (!kSorts.contains(srt) && !m.listSorts().contains(srt)) {
                            // K ::= Sort
                            newProds.add(Constructors.Production(Sorts.K(), Collections.Seq(NonTerminal(srt)), Att()));
                        }
                    }
                    ...
                    for (UserList ul : uLists) {
                        ...
                        prod1 = Constructors.Production(ul.terminatorKLabel, KORE.Sort(ul.sort.localName() + "#Terminator"), Collections.Seq(Constructors.Terminal("")),
                                newAtts.add("klabel", ul.terminatorKLabel).add(Constants.ORIGINAL_PRD, ul.pTerminator));
                        // Ne#Es ::= E "," Ne#Es [klabel('_,_)]
                        prod2 = Constructors.Production(ul.klabel, KORE.Sort("Ne#" + ul.sort.localName()),
                                Collections.Seq(NonTerminal(ul.childSort), Constructors.Terminal(ul.separator), NonTerminal(KORE.Sort("Ne#" + ul.sort.localName()))),
                                newAtts.add("klabel", ul.klabel).add(Constants.ORIGINAL_PRD, ul.pList));
                        // Ne#Es ::= E Es#Terminator [klabel('_,_)]
                        prod3 = Constructors.Production(ul.klabel, KORE.Sort("Ne#" + ul.sort.localName()),
                                Collections.Seq(NonTerminal(ul.childSort), NonTerminal(KORE.Sort(ul.sort.localName() + "#Terminator"))),
                                newAtts.add("klabel", ul.klabel).add(Constants.ORIGINAL_PRD, ul.pList));
                        // Es ::= Ne#Es
                        prod4 = Constructors.Production(ul.sort, Collections.Seq(NonTerminal(KORE.Sort("Ne#" + ul.sort.localName()))));
                        // Es ::= Es#Terminator // if the list is *
                        prod5 = Constructors.Production(ul.sort, Collections.Seq(NonTerminal(KORE.Sort(ul.sort.localName() + "#Terminator"))));

                        newProds.add(prod1);
                        newProds.add(prod2);
                        newProds.add(prod3);
                        newProds.add(prod4);
                        newProds.add(SyntaxSort(KORE.Sort(ul.sort.localName() + "#Terminator")));
                        newProds.add(SyntaxSort(KORE.Sort("Ne#" + ul.sort.localName())));
                        if (!ul.nonEmpty) {
                            newProds.add(prod5);
                        }
                    }
                    ...
                    return Constructors.Module(m.name(), Collections.add(sortKModule, m.imports()), immutable(newProds), m.att());
                }
                return m;
            }
        }).apply(disambM);
...
//                        Module ruleCells = apply(ruleCellsOriginal);
                        Set<Sentence> newProds = stream(m.localSentences()).flatMap(s -> {
                            ...
                            if (s instanceof Production && (s.att().contains("cell"))) {
                                ...
                                if (cfgInfo.isLeafCell(p.sort())) {
                                    ...
                                    body = NonTerminal(KORE.Sort("Bag"));
                                }
                                final ProductionItem optDots = NonTerminal(KORE.Sort("#OptionalDots"));
                                Seq<ProductionItem> pi = Collections.Seq(p.items().head(), optDots, body, optDots, p.items().last());
                                Production p1 = Constructors.Production(p.klabel().get().name(), KORE.Sort("Cell", ModuleName.apply("KCELLS")), pi, p.att());
                                Production p2 = Constructors.Production(KORE.Sort("Cell", ModuleName.apply("KCELLS")), Collections.Seq(NonTerminal(p.sort())));
                                return Stream.of(p1, p2);
                            }
                            if (s instanceof Production && (s.att().contains("cellFragment"))) {
                                Production p = (Production) s;
                                Production p1 = Constructors.Production(KORE.Sort("Cell", ModuleName.apply("KCELLS")), Collections.Seq(NonTerminal(p.sort())));
                                return Stream.of(p, p1);
                            }
                            return Stream.of(s);
                        }).collect(Collectors.toSet());
                        m = Constructors.Module(m.name(), Collections.add(ruleCellsOriginal, m.imports()), immutable(newProds), m.att());
                    }
                }
                ...
                if (extensionM.importedModules().exists(m1 -> m1.name().equals(CONFIG_CELLS)) && !m.name().equals(CONFIG_CELLS)) {
                    ...
                    if (!m.name().equals(CONFIG_CELLS) && !configCellsOriginal.importedModules().exists(m1 -> m1.name().equals(mName))) {
                        ...
                        m = Constructors.Module(m.name(), Collections.add(configCells, m.imports()), immutable(newProds), m.att());
                    }
                }
                    ...
                    Set<Sentence> newProds = stream(m.localSentences()).map(s -> {
                        if (s instanceof Production) {
                            ...
                            if (p.klabel().isDefined())
                                p = Constructors.Production(p.klabel().get().name(), p.sort(), Collections.Seq(items), p.att());
                            else
                                p = Constructors.Production(p.sort(), Collections.Seq(items), p.att());
                            return p;
                        }
                        return s;
                    }).collect(Collectors.toSet());
                    m = Constructors.Module(m.name(), m.imports(), immutable(newProds), m.att());
                }
                ...
                if (extensionM.importedModules().exists(m1 -> m1.name().equals(RULE_LISTS))) {
                    ...
                    for (UserList ul : UserList.getLists(newProds)) {
                        Production prod1;
                        // Es ::= E
                        prod1 = Constructors.Production(ul.sort, Collections.Seq(NonTerminal(ul.childSort)));
                        newProds.add(prod1);
                    }
                    m = Constructors.Module(m.name(), m.imports(), immutable(newProds), m.att());
                }
    ...
    private static Set<Sentence> makeCasts(Sort outerSort, Sort innerSort, Sort castSort) {
        ...
        prods.add(Constructors.Production("#SyntacticCast", castSort, Collections.Seq(NonTerminal(castSort), Constructors.Terminal("::" + castSort.name())), attrs1));
        prods.add(Constructors.Production("#SemanticCastTo" + castSort.name(), castSort, Collections.Seq(NonTerminal(castSort), Constructors.Terminal(":" + castSort.name())), attrs1));
        prods.add(Constructors.Production("#InnerCast", outerSort, Collections.Seq(NonTerminal(castSort), Constructors.Terminal("<:" + castSort.name())), attrs1));
        prods.add(Constructors.Production("#OuterCast", castSort, Collections.Seq(NonTerminal(innerSort), Constructors.Terminal(":>" + castSort.name())), attrs1));
        // annotations with :Sort
        prods.add(Constructors.Production("#SyntacticCast", castSort, Collections.Seq(NonTerminal(castSort), Constructors.Terminal("::" + castSort.localName())), attrs1));
        prods.add(Constructors.Production("#SemanticCastTo" + castSort.name(), castSort, Collections.Seq(NonTerminal(castSort), Constructors.Terminal(":" + castSort.localName())), attrs1));
        prods.add(Constructors.Production("#InnerCast", outerSort, Collections.Seq(NonTerminal(castSort), Constructors.Terminal("<:" + castSort.localName())), attrs1));
        prods.add(Constructors.Production("#OuterCast", castSort, Collections.Seq(NonTerminal(innerSort), Constructors.Terminal(":>" + castSort.localName())), attrs1));
        return prods;
    }
}
